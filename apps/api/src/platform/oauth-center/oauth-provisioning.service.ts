...
/**
 * Post-OAuth provisioning: profile, ads, sync wizard → READY + Live Platform first sync.
 */
@Injectable()
export class OAuthProvisioningService {
  ...
  async provisionAfterConnect(
    ...
    try {
      ...
      try {
        await this.live.triggerFullSync(tenantId, accountId, ctx.correlationId);
        wizardSteps.push({
          name: 'live_sync',
          status: 'completed',
          message: 'Live Platform first sync queued and processed',
        ...
        wizardSteps.push({
          name: 'live_sync',
          status: 'failed',
          message: friendlyAvitoError(e, 'Live sync'),
          completedAt: new Date().toISOString(),
        });
      }
